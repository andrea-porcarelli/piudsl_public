...
function renderActivityCard(act) {
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-amber-100 overflow-hidden">
        ...
        <div class="p-4 pt-2">
            ...
            ${act.address_notes ? `<div style="margin:6px 0;padding:6px 8px;background:#fff8e6;border-left:3px solid #f0ad4e;border-radius:4px;font-size:13px;"><strong>Note:</strong> ${esc(act.address_notes)}</div>` : ''}
    ...
    acts.forEach(act => {
        html += `<button onclick="openActivityDetail('activity', ${act.id})" class="w-full text-left bg-white rounded-2xl shadow-sm border border-amber-100 overflow-hidden active:opacity-70">
            ...
            <div class="p-3">
                ...
                ${act.address_notes ? `<div style="margin:4px 0;padding:4px 6px;background:#fff8e6;border-left:3px solid #f0ad4e;border-radius:4px;font-size:12px;"><strong>Note:</strong> ${esc(act.address_notes)}</div>` : ''}
...
function _buildActivityContent(d) {
    ...
    <div class="space-y-1">
        ...
        ${d.address_notes ? `<div style="margin:8px 0;padding:8px 10px;background:#fff8e6;border-left:3px solid #f0ad4e;border-radius:4px;font-size:13px;"><i data-feather="map-pin"></i> <strong>Note impianto:</strong> ${esc(d.address_notes)}</div>` : ''}
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Allinea anche la card in agenda se presente
                ...
                if (act && json.data?.coordinates) {
                    act.coordinates = json.data.coordinates;
                    act.maps_url = json.data.maps_url;
                    act.waze_url = json.data.waze_url;
                }
